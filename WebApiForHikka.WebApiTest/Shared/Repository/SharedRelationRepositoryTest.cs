...
> : SharedRepositoryTest<TRelationModel, TRelationRepository>
    ...
    where TRelationRepository : CrudRelationRepository<TRelationModel, TFirstModel, TSecondModel>,
    IRelationCrudRepository<TRelationModel, TFirstModel, TSecondModel>
...
{
    protected abstract (TFirstRepository firstRepository, TSecondRepository secondRepository)
        GetFirstAndSecondRepositories(HikkaDbContext hikkaDbContext);

    protected abstract Task<(Guid firstId, Guid secondId)> CreateFirstAndSecondModels(
        (TFirstRepository firstRepository, TSecondRepository secondRepository) repostiroeis);
    ...
    public virtual async Task RepositoryRelation_DeleteByTwoIdsAsync_DeleteModel()
    {
        ...
        var sampleIds = await CreateFirstAndSecondModels(firstAndSecondRepositories);

        var relationModelId =
            await repository.AddAsync(GetRelationModel(sampleIds.firstId, sampleIds.secondId), CancellationToken);
    ...
    [Fact]
    public override async Task Repository_GetAllAsync_ReturnsPage()
    {
        // Arrange
        ...
        for (var i = 0; i < 2; i++)
        {
            var sample = await CreateFirstAndSecondModels(firstAndSecondRepositories);
            sampleIds.Add(sample);
        }

        for (var i = 0; i < 2; i++)
            relationModelsIds.Add(
                await repository.AddAsync(GetRelationModel(sampleIds[i].firstId, sampleIds[i].secondId),
                    CancellationToken));
    ...
    [Fact]
    public override async Task Repository_GetAsync_ReturnsModel()
    {
        // Arrange
        var dbContext = GetDatabaseContext();
        var firstAndSecondRepositories = GetFirstAndSecondRepositories(dbContext);
    ...
    [Fact]
    public override async Task Repository_Get_ReturnsModel()
    {
        // Arrange
        var dbContext = GetDatabaseContext();
        var firstAndSecondRepositories = GetFirstAndSecondRepositories(dbContext);
    ...
    public override async Task Repository_UpdateAsync_UpdateModel()
    {
        ...
        var repository = GetRepository(dbContext);
        var sampleIds = await CreateFirstAndSecondModels(firstAndSecondRepositories);
        var secondSampleIds = await CreateFirstAndSecondModels(firstAndSecondRepositories);
