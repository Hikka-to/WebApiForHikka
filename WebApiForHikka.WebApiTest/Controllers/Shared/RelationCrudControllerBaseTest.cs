...
public abstract class RelationCrudControllerTest<
    ...
    where TController : RelationCrudController<TRelationModel, TFirstModel, TSecondModel,
        TRelationService>
{
    ...
    public virtual async Task CrudRelationController_Create_ReturnsOkObjectResult()
    {
        ...
        var firstId = await serviceProvider.GetRequiredService<TFirstService>()
            .CreateAsync(GetFirstModelSample(), CancellationToken);
        var secondId = await serviceProvider.GetRequiredService<TSecondService>()
            .CreateAsync(GetSecondModelSample(), CancellationToken);


        //Act

        var result =
            await controller.Create(firstId, secondId, CancellationToken) as OkObjectResult;
    ...
    public virtual async Task CrudRelationController_Check_ReturnsNotFoundObjectResult()
    {
        ...
        var result =
            await controller.Check(Guid.NewGuid(), Guid.NewGuid(), CancellationToken) as
                NotFoundResult;
