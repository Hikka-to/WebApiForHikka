...
public static class IQueryableExtensions
{
    public static IQueryable<T> Filter<T>(this IQueryable<T> source, string propertyName, string filter)
    {
        ...
        foreach (var propName in propertyName.Split('.')) body = Expression.PropertyOrField(body, propName);
    ...
    public static IQueryable<T> FilterMany<T>(this IQueryable<T> source, string collectionPropertyName, string filter)
    {
        var param = Expression.Parameter(typeof(T), "e");
        var body = (Expression)param;
        foreach (var propName in collectionPropertyName.Split('.')) body = Expression.PropertyOrField(body, propName);
        ...
        if (anyBody.Type != typeof(string)) anyBody = Expression.Call(anyBody, "ToString", Type.EmptyTypes);
    ...
    public static IQueryable<T> FilterMany<T>(this IQueryable<T> source, string collectionPropertyName,
        string propertyName, string filter)
    {
        ...
        foreach (var propName in propertyName.Split('.')) anyBody = Expression.PropertyOrField(anyBody, propName);

        if (anyBody.Type != typeof(string)) anyBody = Expression.Call(body, "ToString", Type.EmptyTypes);
